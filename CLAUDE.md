...
**Handoff chains (work-graph layer):** `handoff` accepts an optional `chain` arg — an ordered list of steps `{ role? | target?, task, gate? }` (mutually exclusive with `target_slug`/`role`/`message`). Step 1 fires immediately tagged `#h-<id>` + `#c-<id>` + `step 1/N`; when a step's handoff closes (via `handoff_complete` OR bot-reply auto-close), MCD resolves the next step's target against current config and auto-fires it with the prior outcome (≤500 chars) as context, posting `⛓ chain #c-x: n/N done → <next>` to the source channel. `gate: "approve"` halts the chain unless the closing outcome starts with `approve` (trimmed, case-insensitive) — halt escalates to master with the outcome excerpt. Chains longer than the source's effective peers `maxHops` (fallback 6) are refused at creation; a mid-chain step that no longer resolves (renamed slug, disabled project, changed allowlist) halts + escalates. Chain expiry rides the per-step sweep: an expired step handoff expires the chain and the master escalation names the stalled step. Registry file upgraded to `{ version: 2, handoffs, chains }` (legacy bare-array reads fine; chain transitions are one atomic write). `!project collab <slug>` lists open chains with per-step glyphs (`✔` done, `▶` current, `·` unfired, `✖` gate-failed). Single-hop handoffs unchanged; chain auto-advance bypasses bot-peer cooldowns (registry-driven, bounded by hop budget + gates + sweep).
