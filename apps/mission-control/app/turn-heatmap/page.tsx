...
import type { TurnHeatmapResponse, HeatGrid } from '../api/turn-heatmap/route'
...
interface TooltipState { day: number; hour: number; count: number; x: number; y: number }

function HeatGridView({ grid, peakDay, peakHour }: { grid: HeatGrid; peakDay: number; peakHour: number }) {
  ...
  return (
    <div className="relative overflow-x-auto">
      <svg width={W} height={H} className="block" style={{ minWidth: W }}>
        {HOURS.map((h) => h % 3 === 0 && (
          <text key={h} x={LABEL_W + h * CELL_W + CELL_W / 2} y={LABEL_H - 3}
            textAnchor="middle" fill="#475569" fontSize="0.45rem" fontFamily="monospace">
            {String(h).padStart(2, '0')}
          </text>
        ))}
        {DAYS.map((day, di) => (
          <g key={day}>
            <text x={LABEL_W - 4} y={LABEL_H + di * CELL_H + CELL_H / 2 + 4}
              textAnchor="end" fill="#64748B" fontSize="0.5rem" fontFamily="monospace">
              {day}
            </text>
            {HOURS.map((h) => {
              ...
              return (
                <rect key={h}
                  x={LABEL_W + h * CELL_W} y={LABEL_H + di * CELL_H}
                  ...
                  onMouseEnter={(e) => {
                    ...
                    setTooltip({ day: di, hour: h, count, x: r.left + CELL_W / 2, y: r.top })
                  }}
      ...
      {tooltip && (
        <div className="fixed z-50 pointer-events-none rounded border border-white/10 px-2 py-1 text-[0.55rem] font-mono"
          style={{ left: tooltip.x - 60, top: tooltip.y - 36, background: 'rgba(8,15,28,0.95)', backdropFilter: 'blur(6px)', transform: 'translateX(-50%)' }}>
          <span className="text-slate-300">
            {DAYS[tooltip.day]} {String(tooltip.hour).padStart(2, '0')}:00–{String(tooltip.hour + 1).padStart(2, '00')}:00
          </span>
        ...
        <div key={d} className="flex items-center gap-2">
          ...
          <div className="flex-1 h-2 rounded-full bg-slate-800 overflow-hidden">
            <div className="h-full rounded-full"
              style={{ width: `${v > 0 ? Math.max(4, (v / max) * 100) : 0}%`, background: '#22D3EE', opacity: 0.3 + 0.7 * (v / max) }} />
          </div>
...
export default function TurnHeatmapPage() {
  const [data, setData] = useState<TurnHeatmapResponse | null>(null)
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/turn-heatmap?${params}`)
      .then((r) => r.json())
      .then((d: TurnHeatmapResponse) => { setData(d); setLoading(false) })
      .catch(() => setLoading(false))
  }, [windowDays])

  useEffect(() => { setLoading(true); load() }, [load])

  const selected = data?.projects.find((p) => p.slug === selectedSlug)
  const grid = selected?.grid ?? data?.fleet ?? Array.from({ length: 7 }, () => Array(24).fill(0))
  const peakDay = selected?.peakDay ?? data?.fleetPeakDay ?? 0
  const peakHour = selected?.peakHour ?? data?.fleetPeakHour ?? 0
  const total = selected?.total ?? data?.fleetTotal ?? 0

  return (
    <div className="min-h-screen p-4 md:p-6" style={{ background: '#080f1c', color: '#E2E8F0' }}>
      <SubPageHeader title="Session Turn Heatmap">
        <span className="text-[0.6rem] font-mono text-slate-500">
          Claude assistant turns by day-of-week × hour-of-day · last {data?.windowDays ?? 30} days
        </span>
      </SubPageHeader>
      ...
      {!loading && data && (
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-wrap items-center gap-3 mb-5">
            ...
            <select value={selectedSlug ?? ''} onChange={(e) => setSelectedSlug(e.target.value || null)}
              ...
              {data.projects.map((p) => <option key={p.slug} value={p.slug}>{p.slug}</option>)}
            </select>
          </div>

          {data.fleetTotal === 0 ? (
            <div className="text-center py-20 text-slate-600 font-mono text-sm">
              No assistant turns found in transcripts for this window
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_200px] gap-6">
              <div className="rounded-lg border border-white/5 p-4" style={{ background: 'rgba(255,255,255,0.02)' }}>
                <div className="text-[0.6rem] font-mono text-slate-500 uppercase tracking-wider mb-1">
                  {selectedSlug ?? 'Fleet'} — turns by hour (UTC)
                </div>
                <div className="text-[0.55rem] font-mono text-slate-700 mb-3">
                  Peak: {DAYS[peakDay]} {String(peakHour).padStart(2, '0')}:00 UTC
                  · {total.toLocaleString()} total turns
                </div>
                <HeatGridView grid={grid} peakDay={peakDay} peakHour={peakHour} />
              ...
              <div className="rounded-lg border border-white/5 p-4" style={{ background: 'rgba(255,255,255,0.02)' }}>
                <div className="text-[0.6rem] font-mono text-slate-500 uppercase tracking-wider mb-3">By day</div>
                <DayBar grid={grid} />
                {!selectedSlug && data.projects.length > 0 && (
                  <div className="mt-5">
                    <div className="text-[0.6rem] font-mono text-slate-500 uppercase tracking-wider mb-2">Per project</div>
                    <div className="space-y-1.5 max-h-48 overflow-y-auto">
                      {data.projects.map((p) => (
                        <button key={p.slug} onClick={() => setSelectedSlug(p.slug)}
                          ...
                          <span className="text-[0.55rem] font-mono text-slate-400 flex-1 truncate">{p.slug}</span>
                          <span className="text-[0.55rem] font-mono text-cyan-600">{p.total}</span>
                        </button>
                      ))}
                    </div>
                  </div>
