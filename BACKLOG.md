...
- AC6: Added to Intelligence nav; empty state with congratulatory message when no orphans
...
P275 (Project Health Scorecard) displays scores but provides no proactive notification when a score drops below a critical threshold. Operators discover degraded projects only by periodically visiting the scorecard page — reactive, not proactive.

### Proposed Solution

Add a health-score alert rule system. Extend `channels.json` defaults and per-project config with an optional `healthScoreThreshold` field (integer 0-100, default none). The MCD server's existing scheduler tick (60s) calls the health-score computation for each project that has a threshold set; if the score drops below the threshold for two consecutive ticks, it posts a Discord message to the master channel and (if configured) to the project channel. Alert format: `⚠️ [slug] health score dropped to <score>/<threshold> — breakdown: <top 3 failing factors>`. Add a `/api/health-alert-rules` endpoint and a UI section in `/project-config` to view/edit per-project thresholds. No external alerting service needed — routes through the existing MCD master channel reply mechanism.

### Acceptance Criteria

- AC1: `healthScoreThreshold` field supported in `channels.json` per-project and defaults
- AC2: Scheduler tick evaluates health score for each project with a threshold set
- AC3: Alert fires to master channel when score < threshold for 2 consecutive ticks; includes top 3 failing factors
- AC4: Alert de-duplicates: no repeat alerts until score recovers above threshold + 5 (hysteresis)
- AC5: `/project-config` shows threshold input per project; saved via `!project set` or direct config edit
- AC6: `/api/health-alert-rules` returns current rules and last-evaluated scores for all projects
